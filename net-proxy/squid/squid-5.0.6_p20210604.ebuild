...
r=-20210604-r325e90438
#r=-20181117-r0022167
if [ -z "$r" ]; then
	SRC_URI="http://www.squid-cache.org/Versions/v${PV%%.*}/${P}.tar.xz"
else
	MY_P="${PN}-$(ver_cut 1-3)${r}"
	SRC_URI="http://www.squid-cache.org/Versions/v${PV%%.*}/${MY_P}.tar.bz2"
	S="${WORKDIR}/${MY_P}"
fi
...
IUSE="aio caps gnutls ipv6 pam ldap samba sasl kerberos nis radius ssl snmp selinux logrotate test \
	ecap esi ssl-crtd \
...
src_prepare() {
	eapply "${FILESDIR}/${PN}-5.0-gentoo.patch"
